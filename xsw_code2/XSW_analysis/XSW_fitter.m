function [q,err_fh,err_ph] = XSW_fitter(datadir,dataprefix,data_square,theta,plotter,energy,plane,angle,save)
% Function for the fitting of standing wave curves and extracting coherent
% fraction (fh) and position (ph) values. f = q(2), p = q(3).

% Contains %%%%%%%%%% Editable %%%%%%%%%% sections

% Inputs:
%   datadir     - Directory containing .nff files for all elements.
%   dataprefix  - String for saving figure files of final output.
%   data_square - Array containing:
%       [photon energies (hv), fitted peak intensity values, rocking curve data, error on peak intensities]
%   theta       - Theta angle (15 deg).
%   plotter     - 1 or 0 value or whether to plot output (1) or not (2).
%   energy      - Bragg energy.
%   plane       - Reflection plane as an array ie (-111) --> [-1,1,1].
%   angle       - Polar angle of detector.
%   save        - 1 or 0 to save or not save generated figures.
...
% Bodges to make variable global
gaus = 0;
rsga = 0;
desgm = 0;
...
hs = plane;
samp='Ag';  %Sample crystal                                                %%%%%%%%%% Editable %%%%%%%%%%
...
% [bettab,gamtab,deltab,Eb] = q_param(datadir,7,1,1,0);
...
datafile=[datadir dataprefix '.txt'];
dataoutfile=[datadir dataprefix '_dat.txt'];
fitoutfile=[datadir dataprefix '_fit.txt'];
figurefile=[datadir dataprefix '.pdf'];
...
if plotter ==1
figure(1)
subplot('Position',[0.04 0.1 0.2 0.4]);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if plotter == 1
    subplot('Position',[0.32 0.1 0.63 0.88]);
          ...
          %label plots
            ...
            text(0.05,0.75,['                 Filename: ' dataprefix newline newline...
                   'General Information: ' newline...
                   '   Energy = ' num2str(energy) ' eV,      mono = Si(' num2str(hm(1)) ' ' num2str(hm(2)) ' ' num2str(hm(3)) '),' newline... 
                   '   sample = ' samp '(' num2str(hs(1)) ' ' num2str(hs(2)) ' ' num2str(hs(3)) '),     dE/dth = ' num2str(dedth) ' (ev/deg)' newline...
                   'Fitting the Bulk Reflection Profile: ' newline...
                   '   Least-square = ' num2str(v) ',       bs = ' num2str(bs) ',       bm = ' num2str(bm) newline...
                   '   Gaussian width = ' num2str(fwhmgaus) ' (eV),   Gaussian area = ' num2str(areagaus) newline...
                   '   Rmax = ' num2str(max(rsgm)) ',                 I0 = ' num2str(p(3)) '(cts)' newline...
                   '   Energy range = ' num2str(max(datarf(:,1))-min(datarf(:,1))) ' (eV), Angular range = ' num2str((max(datarf(:,1))-min(datarf(:,1)))/dedth) ' (deg)'  newline...
                   'Fitting the XSW Absorption Curve: ' newline...
                   '   Least-square = ' num2str(v2) newline...
                   '   Yob = ' num2str(yob) newline...
                   '   fH = ' num2str(q(2)) ' ± ' num2str(round(err_fh,2,'significant')) newline...
                   '   PH = ' num2str(q(3)) ' ± ' num2str(round(err_ph,2,'significant')) newline...
                   'Non-dipolar corrections used: ' newline...
                   '   C1 = (1+Q)/(1-Q) = ' num2str(C1) newline...
                   '   C2 = 1/(1-Q) = ' num2str(C2) newline...
                   '   C3 = phase shift = ' num2str(C3) newline... 
                   '   Q =  ' num2str(Q) newline],...
                   'interpreter', 'none',...
                   'units','normalized') % 'interpreter', 'none 'allows to do the underscore
            hold off
            if save == 1
                savefig([dataprefix '.fig'])
                saveas(gcf,dataprefix,'png')
