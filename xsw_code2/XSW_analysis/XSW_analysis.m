...
%for -111 where you can only use half of the detector use same half as
%111...
...
for n = 1:size(data_square4d,4) + 1
    ...
    if n <= size(data_square4d,4)
        for nn = 1:length(hv)
            ...
            % For only half of the detector change line below! Due to -111
            % EDIT FOR ELLIE
            if size(data_square4d,1) == 699
                data_export(nn,:) = squeeze(sum(data_square4d(350:699,nn,:,n),1))';
            else
    ...
    % For Averaged data.
        ...
        for nn = 1:length(hv)
            ...
            % EDIT FOR ELLIE
            if size(data_square4d,1) == 699
                data_export(nn,:) = squeeze(sum(data_square(350:699,nn,:),1))';
            else
...
% 
% figure; 
% plot(energy,data_export(1,:)/max(data_export(1,:)));
% hold on;
% plot(energy,data_export(40,:)/max(data_export(40,:)));
% 
% xlabel('Binding energy (eV)');
% ylabel('Intensity (arb.) [normalised to max intensity]');
% set(gca, 'XDir','reverse');
% legend(['hv = ',num2str(hv(1)),'eV (start)'],['hv = ',num2str(hv(40)),'eV (end)'],'Location','best');
% title([filenames,' C1s XSW data 1st and 40th data normalised by max value']);
% saveas(gcf,'1st_40th_XSW_data.png')
% 
% close all;
% error('Done here!');
%%
...
% photon energy.
subplot('position',[0.5, 0.1, 0.45, 0.85])
...
%     bragg = 2630.5;
    ...
    % For close packed N1s 111
%     bragg = 2630.6;
    %Ellie - This works close packed C1s + N1s
    % bragg = 2631.2;
...
%         filename(end) = num2str(peak_no);
    ...
    function run_XSW(source,event)
        ...
        if peak_no <= no_peaks
            ...
            fpfpp_dir = 'C:\Users\ppxcf1\Desktop\Data\I09-beamtime-January\NIXSW analysis guide\XSW code2\XSW_analysis\fpfpp'; %%%%%%%%%% Editable %%%%%%%%%%
            ...
            %Matt edit = 
            %dlmwrite('test_file.txt', [hv',c_out(:,3*(peak_no+1)),err_in'], '\t');
